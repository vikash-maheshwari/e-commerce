...
function Cart() {
  ...
  async function order() {
    ...
    try {
      const response = await fetch(
        "https://json-server-u1lr.onrender.com/orders",
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      <div className="w-screen  mb-10 font-bold text-2xl text-center">
        My Cart
      </div>
      ...
      <div className="flex gap-10 w-11/12 mx-auto mt-4">
        {total > 0 ? (
          <>
            ...
            <div className="w-[50%] flex flex-col   p-4 rounded-sm">
              {cart.map((prod) => (
                <div
                  key={prod.id}
                  className="flex gap-4 mt-8 border-b-2 border-2 w-[70%] bg-white p-4"
                >
                  <img
                  ...
                  <div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <button
                        onClick={() =>
                          dispatch(removeFromCart(prod.product.id))
                        }
                      >
                    ...
                    <div className="flex gap-4 items-center mt-3">
                      <FaPlus onClick={() => add(prod, prod.quantity)} />
                      {prod.quantity}
                      <FaMinus onClick={() => mini(prod, prod.quantity)} />
                    </div>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex flex-col lg:w-[25%] gap-2 mt-20 bg-slate-200 border p-8 h-[10%] ">
          ...
          <div
            className="w-full  flex flex-col items-center justify-center text-4xl "
            style={{ height: `calc(100vh - 12.5rem)` }}
          >
            No items
            <p className="text-2xl cursor-pointer mt-4 bg-yellow-400 p-2 rounded-2xl">
